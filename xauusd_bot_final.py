...
def generate_signal(prices):
    ...
    # تحديد الإشارة
    if rsi[last] < 35 and macd[last] > macd_sig[last]:
        signal = "BUY"
        direction = "صعود 📈"
        strength = min(60 + (35 - rsi[last]) * 2, 95)
    elif rsi[last] > 65 and macd[last] < macd_sig[last]:
        signal = "SELL"
        direction = "نزول 📉"
        strength = min(60 + (rsi[last] - 65) * 2, 95)
    else:
        signal = "NEUTRAL"
        direction = "محايد ↔️"
        strength = 35
...
def send_telegram(signal_data):
    ...
    names = {"BUY": "شراء", "SELL": "بيع", "NEUTRAL": "محايد"}
    
    signal = signal_data["signal"]
    
    message = f"""{emojis.get(signal, "⚪")} <b>إشارة XAUUSD - {names.get(signal, "محايد")}</b> {emojis.get(signal, "⚪")}
...
📍 <b>المستويات:</b>
• الدعم: ${signal_data['support']}
• المقاومة: ${signal_data['resistance']}
...
def main():
    ...
    while True:
        try:
            ...
            # إرسال
            if first_run:
                print("  🚀 إرسال أول إشارة فوراً...")
                send_telegram(signal)
                last_signal = signal["signal"]
                first_run = False
            elif signal["signal"] != last_signal or signal["strength"] > 55:
                print("  🔄 تغير الإشارة - إرسال...")
                send_telegram(signal)
                last_signal = signal["signal"]
            else:
                print(f"  ⏭️  نفس الإشارة ({signal['signal']}) - تخطي")
