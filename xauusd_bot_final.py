...
def main():
    ...
    while True:
        try:
            ...
            # 🚀 أول إشارة فوراً عند التشغيل
            if first_run:
                print(f"  🚀 أول إشارة: {signal['signal']} - إرسال...")
                send_telegram(signal)
                last_signal = signal["signal"]
                last_sent_time = time.time()
                first_run = False
            # 🔄 إرسال عند تغير الإشارة
            elif signal["signal"] != last_signal:
                print(f"  🔄 تغير الإشارة: {last_signal} → {signal['signal']} - إرسال...")
                send_telegram(signal)
                last_signal = signal["signal"]
                last_sent_time = time.time()
            else:
                print(f"  ⏭️  نفس الإشارة ({signal['signal']}) - لا إرسال")
